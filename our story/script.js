...
function updateHomepagePhoto() {
    ...
    const filenameSpan = document.getElementById('homepage-photo-filename');
    if (filenameSpan) filenameSpan.innerText = photoFilename;
}

function handleHomepageFileSelect(event) {
    const file = event.target.files && event.target.files[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = function(e) {
        const dataUrl = e.target.result;
        localStorage.setItem('homepagePhoto', dataUrl);
        const display = document.getElementById('homepage-photo-display');
        if (display) {
            display.innerHTML = `<img src="${dataUrl}" style="width:100%; aspect-ratio:1; object-fit:cover;">`;
        }
        const fileInput = document.getElementById('homepage-photo-file');
        const filenameSpan = document.getElementById('homepage-photo-filename');
        if (fileInput) fileInput.value = "";
        if (filenameSpan) filenameSpan.innerText = file.name || 'Selected';
    };
    reader.readAsDataURL(file);
}

function handleHomepageFileDrop(event) {
    event.preventDefault();
    const file = event.dataTransfer && event.dataTransfer.files && event.dataTransfer.files[0];
    if (!file) return alert("No file dropped.");
    const reader = new FileReader();
    reader.onload = function(e) {
        const dataUrl = e.target.result;
        localStorage.setItem('homepagePhoto', dataUrl);
        ...
        const filenameSpan = document.getElementById('homepage-photo-filename');
        if (filenameSpan) filenameSpan.innerText = file.name || 'Dropped';
    };
    reader.readAsDataURL(file);
}
